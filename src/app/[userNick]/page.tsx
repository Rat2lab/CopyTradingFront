...
export default function Home({ params }: { params: { userNick: string } }) {
  ...
  const { setActualProfile, actualProfile, loggedUser } = useUserContext();
  const checkProfileEditable = () => {
    return loggedUser && loggedUser.id === actualProfile?.userId;
  };
  ...
  return (
    <div className="max-h-screen relative md:grid grid-cols-12 items-start justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-8 font-[family-name:var(--font-geist-sans)]">
      ...
      <ProfileView mobileMode={false} profile={actualProfile} editable={false} />
    </div>
  );
}
